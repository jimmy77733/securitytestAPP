...
- **用途**：執行檔會在背景啟動本機 HTTP 伺服器，並（可選）自動開啟預設瀏覽器連到 `http://localhost:4173`。實際操作介面在瀏覽器內，執行檔只負責提供網頁與 API，不內建視窗程式。
- **更新內容**：執行檔**不會**把前端或題庫包進 exe，每次執行都是從「與 exe 同一個資料夾」讀取 `dist/`、`public/`。因此只要在該資料夾內替換成新的 `dist`、`public`（例如重新建置後覆蓋），下次執行同一個 exe 就會使用新內容。
